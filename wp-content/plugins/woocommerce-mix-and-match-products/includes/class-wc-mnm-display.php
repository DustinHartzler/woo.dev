...
class WC_Mix_and_Match_Display {
	...
	public function __construct() {
		...
		// Fix woocommerce_single_product_summary price in per product pricing mode
		add_action( 'woocommerce_single_product_summary', array( $this, 'modify_get_price' ), 9 );
		add_action( 'woocommerce_single_product_summary', array( $this, 'reset_get_price' ), 11 );
	 ...
	 * @return void
	 */
	function modify_get_price() {

		global $product;

		// Sync now to ensure no get_price() calls are inadvertently filtered during sync
		if ( $product->is_type( 'mix-and-match' ) && ! $product->is_synced() ) {

			$product->sync();
			add_filter( 'woocommerce_mnm_get_price', array( $this, 'get_single_product_mnm_price' ), 10, 2 );
		}
	}
	 ...
	 * @return void
	 */
	function get_single_product_mnm_price( $price, $mnm ) {

		return $mnm->min_price;
	}
	 ...
	 * @return void
	 */
	function reset_get_price() {

		remove_filter( 'woocommerce_mnm_get_price', array( $this, 'get_single_product_mnm_price' ), 10, 2 );
	}
