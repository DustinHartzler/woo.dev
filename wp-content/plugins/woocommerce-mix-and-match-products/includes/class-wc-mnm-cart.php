...
class WC_Mix_and_Match_Cart {
	...
	public function add_to_cart_validation( $passed_validation, $product_id, $product_quantity, $variation_id = '', $variations = array(), $cart_item_data = array() ) {
		...
		if ( $product->is_type( 'mix-and-match' ) ) {
			...
			if ( $limit === 0 && $mnm_stock->get_total_quantity() === 0 ) {
			...
			} elseif ( $limit > 0 && $mnm_stock->get_total_quantity() > $limit ) {
				$error_message = sprintf( __( 'You have selected too many items. Please choose %d items for &quot;%s&quot;.', 'woocommerce-mix-and-match-products' ), $limit, $product->get_title() );
			} elseif ( $limit > 0 && $mnm_stock->get_total_quantity() < $limit ) {
				$error_message = sprintf( __( 'You have selected too few items. Please choose %d items for &quot;%s&quot;.', 'woocommerce-mix-and-match-products' ), $limit, $product->get_title() );
			}
	 ...
	 * @return array
	 */
	private function get_mnm_cart_item( $cart_item, $parent ) {
	...
	public function add_cart_item_filter( $cart_item, $cart_item_key ) {
		...
		if ( ! empty( $cart_item[ 'mnm_container' ] ) ) {
			...
			if ( WC()->cart->find_product_in_cart( $container_cart_key ) ) {
				...
				$cart_item = $this->get_mnm_cart_item( $cart_item, $parent );
