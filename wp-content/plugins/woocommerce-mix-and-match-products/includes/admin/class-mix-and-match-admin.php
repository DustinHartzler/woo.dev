...
class WC_Mix_and_Match_Admin {
	...
	/**
	 * @var types of products that can currently go in a container
	 */
	...
	public static function init() {
		...
		// Per product pricing and Non-bundled shipping options
	...
	public static function type_options( $options ) {
		...
		$options[ 'mnm_per_product_shipping' ] = array(
			...
			'label' 		=> __( 'Non-Bundled Shipping', 'woocommerce-mix-and-match-products' ),
			'description' 	=> __( 'If your Mix-and-Match product consists of items that are assembled or packaged together, leave the box un-checked and just define the shipping properties of the product below. If, however, the chosen items are shipped individually, their shipping properties must be retained. In this case, the box must be checked.', 'woocommerce-mix-and-match-products' ),
			'default'		=> 'no',
		);
	...
	public static function process_meta( $post_id ) {
		...
		if ( isset( $_POST[ 'mnm_allowed_contents' ] ) && ! empty( $_POST[ 'mnm_allowed_contents' ] ) ) {
			...
			foreach ( $mnm_allowed_contents as $mnm_id ) {
				...
				if ( ! in_array( $product->product_type, self::$supported_types ) || ( $product->is_type( 'variation' ) && ! $product->has_all_attributes_set() ) ) {
